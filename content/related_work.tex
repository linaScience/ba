...
This chapter gives a summary of the important research,\fmhkn{wirklich dieses Komma?} that are related to this thesis.



\section{Handover Prediction}

Montavont et al.~\cite{handover-assisted-by-gps}\fmhkn{non-breaking space nach al. -- in jedem Fall aber ein normal space. Siehe frenchspacing. } propose a handover decision algorithm based on the \ac{gps} location of the mobile device.
...
Their \ac{ap} selection predictions outperform the current approaches of strongest received signal first by 9.2\,\% \fmhkn{short space between number and unit} and least-loaded signal\fmhkn{was ist denn ein least-loaded signal? meinen Sie AP statt signal?} first by 8\,\%.
