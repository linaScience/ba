...
For this thesis, the submission files and the test folder will not be used.
...
To further know, how many data we have for the model, \Cref{tab:data_summary} shows a more detailed analysis.
...
But, they can be combined using linear interpolation.%, as seen in \Cref{fig:interpolation}.
...
As in \Cref{fig:vis-wo-interpolated} shows, some waypoints seem to be very distant from the next one, which can be detected by the dotted lines which go all across the floor.
...
Therefore, waypoints which are more than about 5.57 meters apart, are defined as ``to apart from each other'' to walk in this time and therefore, we will split the path there and will generate separated files.
This results in 147 files with data interpolation, which will be used for the \ac{ml} model.
But the data in those interpolated files do not contain any information about the \ac{bssid} and corresponding \ac{rssi} values, which are needed for the prediction.
...
If an \ac{ap} is not present, we add a very low value \(-999\) into the field. 
...
These files will be used to train the \ac{ml} model.
