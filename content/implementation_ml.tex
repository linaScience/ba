...
\fmhkn{obviously, this is a bit too brief in this form :-) }
...
\begin{itemize}
    ...
    \item Explain what was done to the .txt files in preprocessing part  (see \cite{github-repo} preparation.ipynb) \fmhkn{yes, please! in more detail than in the earlier section! }
    ...
    \item Encode the target variable, which is a variable with 4795, \fmhkn{4795 whats?} where 1 means the class is the nearest \ac{ap} and 0 means the class is not the nearest \ac{ap}, which results in a one-hot encoding. \fmhkn{why one-hot? ios this then still mutli-variate?}
...
\begin{itemize}
    ...
    \item Get top 3 predictions for the test set and check if the target variable is in the top 3 predictions.\fmhkn{why top 3 ? why not 2 or 5?}
\end{itemize}
...
\end{figure}
\fmhkn{uh... NIE pngs einbinden; immer Vektorformate!! }
...
\fmhkn{again: why these??? always give reasons for such decisions! }
