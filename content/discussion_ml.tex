...
Building upon the insights from \Cref{ch:data-ana}, we recognize that the indoor location dataset, particularly from the Yintai City (Chengxi Branch) floor, presents us with 4795 \acp{bssid}, translating into a high-dimensional classification problem with 4795 classes.
...
% In conclusion, the dataset's unique characteristics, as detailed in \Cref{ch:data-ana}, necessitate a model that can effectively capture temporal patterns, scale in capacity, handle multivariate data, and offer flexibility.
% Among the models discussed, \acp{lstm} emerge as the most suitable choice, given their ability to handle these requirements adeptly.
% Thus, \acp{lstm} are chosen for our implementation.
