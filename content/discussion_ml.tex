...
\begin{itemize}
    ...
    \item As this is a time series, we want to use a  machine-learning model suitable for time series \fmhkn{kind of obvious, isnt it?}
    \item As we want to predict the next BSSID out of many BSSIDs, we have multi-class classification\fmhkn{why is it multi-class if you just want THE (single) next one??} problem with supervised learning, as we know which BSSID is the next one
    \item Discussion of some pre-chosen models and decision for one which will be implemented in the next chapter \fmhkn{indeed!}
\end{itemize}

\section{Classification Models}
\begin{itemize}
    \item Stated in \cref{sec:background}, classification model in ML is a type of predictive model that categorizes incoming input data into specific classes\fmhkn{but do we really have a classification here? in the narrow sense? classes of APs are given}
    \item Prediction of next BSSID is a classification problem\fmhkn{so, in the sense of classifying the sequence of recent APs into the proper class for ``next AP'' (or next APs)? }
    \item therefore, interpret the problem as a classification problem \fmhkn{is that the common approach? I suppose, but back it up by literature!}
...
\fmhkn{in headings, use full forms of the acronyms}
...
\ac{hmm} is a statistical model that assumes the system being modeled is a Markov process with unobserved (hidden) states \cite{hmm-rabiner-1989}. \fmhkn{always a space before a cite }
...
We will discuss the classification models according to  the following aspects\fmhkn{properties?}: Temporal Dependency Handling, Capacity and Complexity, Multivariate Data, Flexibility and Integration, and Regularization and Overfitting.\fmhkn{Why these? Why are these aspects relevant for the problem at hand? Which are less important? Do you have a concrete expectation for these aspects BEFORE looking at the different techniques? Make it specific to YOUR problem, not just a generic discussion! }

\fmhkn{never use linebreaks in text. No colons at the end of headings. NEver use textbf as headings. Use descipriotn environments or simiarl. }
...
\begin{itemize}
    ...
    \item With 4,795 classes, the model needs a considerable capacity to differentiate between the subtle differences in patterns that might exist among them. \acp{lstm}, being deep learning models, can scale effectively in terms of capacity by adding more layers or units while still maintaining their ability to handle temporal data.\fmhkn{that is the only really concrete, specific aspect, I think? anything I missed? }
\end{itemize}
...
\fmhkn{well... this almost sounds like a foregone conclusion? Empahsis which aspects of the problem really caused this decision. }
